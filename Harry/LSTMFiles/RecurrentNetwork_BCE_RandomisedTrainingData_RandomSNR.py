...
class ModelHandler():
    ...
    def setupDNN(self, input, targets, loss):
        self.model = keras.models.Sequential([
...
def prepare_data2(raw_data, y, timesteps, lookahead):
    n_frames = raw_data.shape[0]
    n_bands = raw_data.shape[1]
    data = np.empty((n_frames-timesteps+1, timesteps, n_bands))
    targets = np.empty((n_frames-timesteps+1, 1))
...
if __name__ == '__main__':
    ...
    speech_input_dir = 'E:\\TrainingData\\csvs\\randomNewData'
            ...
            for entry in entries:
                if entry.is_file():
                    ...
                    if 'Noisen%d-' % i in entry.name:
                        ...
                        x_train_tmp, targets_tmp, dnn_targets_tmp = prepare_data2(data, y, timesteps=timesteps, lookahead=lookahead)
                        ...
                        x_train_dnn = np.concatenate((x_train_dnn, scale_data(data[timesteps-1:], -50.0, 20.0)))

                    #elif 'Track%d.' % track_val in entry.name and 'Noisen%d-' % i in entry.name:
                    elif 'Noisen%d-' % i in entry.name:
                        ...
                        #x_val_tmp, targets_val_tmp, dnn_targets_val_tmp = prepare_data(val_data, val_y, timesteps=timesteps,
                                                                                       #lookahead=lookahead)
                        x_val_tmp, targets_val_tmp, dnn_targets_val_tmp = prepare_data2(val_data, val_y, timesteps=timesteps,
                                                                                       lookahead=lookahead)
                    ...
                    if entry.is_file():
                        if 'Track%d.' % track in entry.name and 'Noisen%d-' % i in entry.name:
                            ...
                            #x_train_tmp, targets_tmp, dnn_targets_tmp = prepare_data(data, y, timesteps=timesteps,
                                                                                     #lookahead=lookahead)
                            x_train_tmp, targets_tmp, dnn_targets_tmp = prepare_data2(data, y, timesteps=timesteps,
                                                                                     lookahead=lookahead)
                            ...
                            #x_val_tmp, targets_val_tmp, dnn_targets_val_tmp = prepare_data(val_data, val_y,
                                                                                           #timesteps=timesteps,
                                                                                           #lookahead=lookahead)
                            x_val_tmp, targets_val_tmp, dnn_targets_val_tmp = prepare_data2(val_data, val_y,
                                                                                           timesteps=timesteps,
                                                                                           lookahead=lookahead)
                            x_val = np.concatenate((x_val, x_val_tmp), axis=0)
                            targets_val = np.concatenate((targets_val, targets_val_tmp), axis=0)
                            dnn_targets_val = np.concatenate((dnn_targets_val, dnn_targets_val_tmp), axis=0)
                            x_val_dnn = np.concatenate((x_val_dnn, scale_data(data[timesteps - 1:], -50.0, 20.0)))

    print(x_train.shape)
    x_train_shuff, x_val_shuff,  targets_shuff, targets_val_shuff = train_test_split(x_train, targets, shuffle=True)
    x_train_shuff_dnn, x_val_shuff_dnn, targets_shuff_dnn, targets_val_shuff_dnn = train_test_split(x_train_dnn,
                                                                                                    dnn_targets,
                                                                                                    shuffle=True)
    print(x_train.shape)
    print(targets.shape)
